...
#include "can.h"
 ...
 HAL_StatusTypeDef status;
  		...
  		TxMess.StdId = 0x600 + id1; // 0x600 + Node ID
  		TxMess.DLC = size;
  		TxMess.IDE = CAN_ID_STD;
  		TxMess.RTR = CAN_RTR_DATA;
  		TxMess.Data[0] = id2;
  		TxMess.Data[1] = dataTx[0];
  		TxMess.Data[2] = dataTx[1];
  		TxMess.Data[3] = dataTx[2];
  		TxMess.Data[4] = dataTx[3];
  		TxMess.Data[5] = dataTx[4];
  		TxMess.Data[6] = dataTx[5];
  		TxMess.Data[7] = dataTx[6];
  		hcan.pTxMsg = &TxMess;
 ...
 int CAN_Receive(uint16_t id,uint32_t CAN_FIFON, int size,CAN_HandleTypeDef hcan ){
	 ...
	 CanRxMsgTypeDef RxMess;
	 	...
	 	RxMess.FMI = 14;  //???
	 	RxMess.StdId =  0x580 + id; // 0x580 + Node ID
	 	RxMess.DLC = size;
...
void Send2GUI(uint16_t data[108], int id,  int size,CAN_HandleTypeDef hcan,float adcV, float current){
	...
	for(int i=0;i<27;i++){
		...
		//CAN_Send(id,dataOther,size,hcan);
