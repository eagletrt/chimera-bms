...
//		// Output_low
//		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_3, GPIO_PIN_RESET);
////		spi_write_read(cmd,4,data,8,hsp
...
uint8_t ltc6804_rdcv_temp(uint8_t ic_n,				// Number of the current ic
						  uint8_t parity,
	...
	if(pec15(6, data, crcTable) == (uint16_t) (data[6]*256 + data[7])){
		if(parity==0){
			...
			cell_temp[ic_n*9+1][0] = convert_temp(convert_voltage(&data[2]));
			cell_temp[ic_n*9+1][1] = 0;
		}else{
			cell_temp[ic_n*9][0] = convert_temp(convert_voltage(&data[0]));
			cell_temp[ic_n*9+2][0] = convert_temp(convert_voltage(&data[4]));
			cell_temp[ic_n*9][1] = 0;
			cell_temp[ic_n*9+2][1] = 0;
		}
	}
	else{
		error++;
		if(parity==0){
			cell_temp[ic_n*9+1][1]++;
		}else{
			cell_temp[ic_n*9][1]++;
			cell_temp[ic_n*9+2][1]++;
		}
	...
	if(pec15(6, data, crcTable) == (uint16_t) (data[6]*256 + data[7])){
			if(parity==0){
				cell_temp[ic_n*9+3][0] = convert_temp(convert_voltage(&data[0]));
				cell_temp[ic_n*9+3][1] = 0;
			}else{
				cell_temp[ic_n*9+4][0] = convert_temp(convert_voltage(&data[2]));
				cell_temp[ic_n*9+4][1] = 0;
			}
		}
		else{
			error++;
			if(parity==0){
				cell_temp[ic_n*9+3][1]++;
			}else{
				cell_temp[ic_n*9+4][1]++;
			}
	...
	if(pec15(6, data, crcTable) == (uint16_t) (data[6]*256 + data[7])){
			if(parity==0){
				cell_temp[ic_n*9+5][0] = convert_temp(convert_voltage(&data[0]));
				cell_temp[ic_n*9+7][0] = convert_temp(convert_voltage(&data[4]));
				cell_temp[ic_n*9+5][1] = 0;
				cell_temp[ic_n*9+7][1] = 0;
			}else{
				cell_temp[ic_n*9+6][0] = convert_temp(convert_voltage(&data[2]));
				cell_temp[ic_n*9+6][1] = 0;
			...
			error++;
			if(parity==0){
				cell_temp[ic_n*9+5][1]++;
				cell_temp[ic_n*9+7][1]++;
			}else{
				cell_temp[ic_n*9+6][1]++;
	...
	if(parity==1){
		...
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_3, GPIO_PIN_SET);
		if(pec15(6, data, crcTable) == (uint16_t) (data[6]*256 + data[7])){
			cell_temp[ic_n*9+8][0] = convert_temp(convert_voltage(&data[0]));
			cell_temp[ic_n*9+8][1] = 0;
		}else{
			error++;
			cell_temp[ic_n*9+8][1]++;
		}
	}
	return error;
}
...
/*----- Read the raw data from the ltc6804 cell voltage register normal-----*/
uint8_t ltc6804_rdcv_voltages(uint8_t ic_n,				// Number of the current ic
				  SPI_HandleTypeDef hspi1,
