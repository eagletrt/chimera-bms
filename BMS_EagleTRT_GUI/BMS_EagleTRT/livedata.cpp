...
void LiveData::parseMessage()
{
    ...
    while(serial.canReadLine())
    {
        ...
        if(valuesReceived.length() != 9)
            return;

        qDebug() << valuesReceived << endl;
        if(true)
        {
            ...
            */
            ...
            uint8_t blockNumber = valuesReceived[1].at(0).unicode() - 15;
            qDebug() << blockNumber;
        ...
        //NON E' NECESSARIA STA ROBA
          ...
          /*_______________*/
            ...
            int row1 = (blockNumber) / NCX;
            int col1 = (blockNumber) % NCX;
            int row2 = (blockNumber + 1) / NCX;
            int col2 = (blockNumber + 1) % NCX;
            int row3 = (blockNumber + 2) / NCX;
            int col3 = (blockNumber + 2) % NCX;

            qDebug() << "Row " << row1 << " Col " << col1 << endl;

            addVoltage(valuesReceived[2].at(0).toLatin1() , ui->tableVoltage, row1, col1);
            addVoltage(valuesReceived[4].at(0).toLatin1(), ui->tableVoltage, row2, col2);
            addVoltage(valuesReceived[6].at(0).toLatin1(), ui->tableVoltage, row3, col3);
            ...
            */
            ...
            addTemperature(valuesReceived[3].at(0).toLatin1(), ui->tableTemp, row1, col1);
            addTemperature(valuesReceived[5].at(0).toLatin1(), ui->tableTemp, row2, col2);
            addTemperature(valuesReceived[7].at(0).toLatin1(), ui->tableTemp, row3, col3);
...
void LiveData::on_pushButton_clicked()
{
    openSerial();
    serial.clear();
