 ...
 *
 * @param		spi				The SPI configuration
 * structure
 ...
 * 					lasts 600ms: too slow for the BMS. To avoid stopping
 * for that long, everytime this function is called only two LTCs are polled,
 * and only even or odd cells are measured from them. When all LTCs have been
 * polled, we start from the beginning and we flip the odd/even bit to read the
 * remaining cells. This decreases the update frequency on a single cell, but
 * greatly improves (~10x) the blocking time of temperature measurements. The
 * time to update a single cell can be calculated as following:
 * 2*CELL_COUNT*(52ms+CALL_INTERVAL)
 *
...
/**
 * @brief		Checks if there are cells that have a higher than
 * average voltage drop under load
 *
 * @details	When called during an idle period (current draw under a certain
 * 					amount), this function stores the total voltage as
 * a reference "idle voltage". When under load, this function compares the total
 * voltage to the idling voltage and if it sees a drop, checks for every cell
 * whether the drop in voltage is higher than average.
 *
 * @param		pack		The PACK_T to check
 * @param		cells		The array of indexes that are found to
